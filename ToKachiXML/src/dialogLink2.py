...
class DialogLink2(object):
    ...
    def set_format(self, row):
        Label(self.dialog, text="参照元") \
                .grid(row=row, column=0)
        Label(self.dialog, text="（政令で定めるetc）") \
        ...
        self.var_banme = IntVar(self.dialog)
        self.var_banme.set(1)
        self.spin_banme = Spinbox(self.dialog,
                textvariable=self.var_banme,
                from_=0, to=20, increment=1)
        self.spin_banme.grid(row=row+6, column=1,
                sticky=W)
        Label(self.dialog, text="番目") \
                .grid(row=row+6, column=2, sticky=N)

        Label(self.dialog, text="") \
                .grid(row=row+7, column=3, sticky=N)
        row += 8

        Label(self.dialog, text="参照先") \
                .grid(row=row, column=0)
        Label(self.dialog, text="（第Ｘ条に規定する") \
                .grid(row=row+1, column=0)
        Label(self.dialog, text="～政令で定めるetc）") \
                .grid(row=row+2, column=0)
...
# [(1, ('措置法施行＿令第４０条の２第４項', ((40, 2), 4, None)))]
        ...
        item = (banme, (saki_link, saki_tuple))
